...
UnitAI.prototype.UnitFsmSpec = {
	...
	"INDIVIDUAL": {
		...
		"FLEEING": {
			"enter": function() {
				// We use the distance between the entities to account for ranged attacks. I replaced position helper with obstruction manager because the datas conflicted and caused bugs
				let cmpObstructionManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_ObstructionManager);
			...
			"APPROACHING": {
				...
				"MovementUpdate": function(msg) {
					...
					if (this.CheckTargetAttackRange(this.order.data.target, this.order.data.attackType))
					{
						...
						if(this.IsRider())
						{
							let horseAI = Engine.QueryInterface(this.IsRider(), IID_UnitAI);
							horseAI.StopMoving();
						}
				...
				"enter": function() {
					if(this.IsRider())
					{
						...
						if (cmpUnitAI && cmpUnitAI.IsFleeing())
							cmpTurAI.SetSpeedMultiplier(this.GetRunMultiplier());
						this.StartTimer(1000, 1000);
						return false;
					}
					if (!this.MoveToTargetAttackRange(this.order.data.target, this.order.data.attackType) )
...
UnitAI.prototype.IsRider = function(cmpTurretable)
{
	if (!cmpTurretable)
		cmpTurretable = Engine.QueryInterface(this.entity, IID_Turretable);
	if(cmpTurretable)
		return cmpTurretable.Is_rider();
	else
		return 0;
};
UnitAI.prototype.IsRiden = function(cmpTurretHolder)
{
	if (!cmpTurretHolder)
		cmpTurretHolder = Engine.QueryInterface(this.entity, IID_TurretHolder);
	if(cmpTurretHolder)
		return cmpTurretHolder.Is_riden();
	else
		return 0;
};
...
{
	if (this.isImmobile && !this.IsRider())
		return false;
