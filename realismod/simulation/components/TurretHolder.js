...
class TurretHolder
{
	...
	AllowedToOccupyTurretPoint(entity, turretPoint)
	{
		...
		if (!IsOwnedByMutualAllyOfEntity(entity, this.entity) && this.template.Available != "true")
			return false;
	...
	TurretPointIsRider(turretPoint)
	{
		if (!turretPoint)
			return false;

		if (turretPoint.Rider != "true")
			return false;
	...
	/**
	 * @param {number} entity - The entity's id.
	 * @return {Boolean} - Whether this enitity is rider.
	 */
	Is_rider(entity)
	{
		return this.TurretPointIsRider(this.GetOccupiedTurretPoint(entity))
	}
	Is_riden()
	{
		for (let turretPoint of this.turretPoints)
			if (turretPoint.Rider == "true" && turretPoint.entity)
				return turretPoint.entity;
		return 0;
	}
	/**
	 * @param {number} entity - The entity's id.
...
TurretHolder.prototype.Schema =
		...
		"<oneOrMore>" +
			"<element a:help='Element containing the offset coordinates.'>" +
				...
				"<interleave>" +
					...
					"</optional>" +
					"<optional>" +
						"<element name='Rider' a:help='Whether this turret can give orders to this entity'>" +
							"<data type='boolean'/>" +
						"</element>" +
					"</optional>" +
					"<optional>" +
	...
	"</element>" +
	"<optional>" +
		"<element name='Available' a:help='Whether it can hold non Ally units (used for horses)'>" +
			"<data type='boolean'/>" +
		"</element>" +
	"</optional>" +
	"<optional>" +
