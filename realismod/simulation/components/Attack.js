...
Attack.prototype.CanAttack = function(target, wantedTypes)
{
	...
	{
		if (type != "Capture" && (this.entity == target || !cmpHealth || !cmpHealth.GetHitpoints()))
			continue;
...
Attack.prototype.PerformAttack = function(type, target)
{
	...
	if (this.template[type].Projectile)
	{
		...
		//Add spread based on Horse's speed
		let addspread = 0;
		...
		if (cmpTurretable)
		{
			...
			if (tur)
			{
				let cmpTuretUnitMotion = Engine.QueryInterface(tur, IID_UnitMotion);
				if (cmpTuretUnitMotion)
				{
					addspread = cmpTuretUnitMotion.GetCurrentSpeed() / 10;
				}
			}
		}
		// Add inaccuracy based on spread.
		let distanceModifiedSpread = (ApplyValueModificationsToEntity("Attack/" + type + "/Spread", +this.template[type].Projectile.Spread, this.entity) + addspread) *
