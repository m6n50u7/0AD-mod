...
AlertRaiser.prototype.Schema =
	...
	"<optional>" +
		"<element name='Garrison'><data type='boolean'/></element>" +
	"</optional>";
...
AlertRaiser.prototype.Init = function()
{
	...
	this.lastTime = 0;
	this.garrison = this.template.Garrison != "false";
};
...
AlertRaiser.prototype.RaiseAlert = function()
{
	...
	else
	{
		...
		let units = cmpRangeManager.ExecuteQuery(this.entity, 0, +this.template.RaiseAlertRange, players, IID_UnitAI, true).filter(ent => this.UnitFilter(ent));
		for (let unit of units)
		{
			let cmpUnitAI = Engine.QueryInterface(unit, IID_UnitAI);
			cmpUnitAI.WalkToTarget(this.entity, false, false);
		}
	}
};
