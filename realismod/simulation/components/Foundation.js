...
Foundation.prototype.GetNeededResourceCount = function(type)
{
	return this.remres[type];
}
Foundation.prototype.ReduceNeededResourceCount = function(type, amount)
{
	this.remres[type] = this.remres[type] - amount;
}
Foundation.prototype.GetTotalResourceCount = function()
{
	return this.tot;
}
Foundation.prototype.GetResourceCosts = function()
{
	let cmpCost = Engine.QueryInterface(this.entity, IID_Cost);
	return cmpCost.GetResourceCosts();
}
...
Foundation.prototype.InitialiseConstruction = function(template)
{
	...
	this.remres = this.costs;
	for (let r in this.costs)
	{
		this.tot += this.costs[r];
	}
	let cmpOwnership = Engine.QueryInterface(this.entity, IID_Ownership);
	let owner = cmpOwnership.GetOwner();
	let cmpPlayer = QueryPlayerIDInterface(owner);
	let cmpStatisticsTracker = QueryPlayerIDInterface(owner, IID_StatisticsTracker);

	// since resource consumption is hardcoded, Refund the costs at the begining. so the resources will be consumed from the builders instead
	for (let r in this.costs)
	{
		if (this.costs[r])
		{
			if (cmpPlayer)
				cmpPlayer.AddResource(r, this.costs[r]);
			if (cmpStatisticsTracker)
				cmpStatisticsTracker.IncreaseResourceUsedCounter(r, -this.costs[r]);
		}
	}
	this.maxProgress = 0;
...
Foundation.prototype.OnHealthChanged = function(msg)
{
	...
	if(this.lastprogress > this.GetBuildProgress())
	{
		for (let r in this.remres)
		{
			this.remres[r] += Math.round((this.costs[r] * (this.lastprogress - this.GetBuildProgress())));
		}
	}	
	this.lastprogress = this.GetBuildProgress();
};
...
Foundation.prototype.GetResRate = function()
{
	let cmpCost = Engine.QueryInterface(this.entity, IID_Cost);
	// Return infinity for instant structure conversion
	return this.GetTotalResourceCount() / cmpCost.GetBuildTime();
};
/**
