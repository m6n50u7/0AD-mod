...
UnitAI.prototype.UnitFsmSpec = {
	...
	"Order.Attack": function(msg) {
		...
		//}
		//let Tur = Engine.QueryInterface(tur, IID_UnitAI);
		//if(Tur)
		//	Tur.WalkToTarget(msg.data.target, !!msg.data.force);
		...
		"COMBAT": {
			...
			"APPROACHING": {
				"enter": function() {
					...
					let Tur = Engine.QueryInterface(tur, IID_UnitMotion);
					let cmpAttack = Engine.QueryInterface(this.entity, IID_Attack);
					if(tur && Tur && PositionHelper.DistanceBetweenEntities(this.entity, this.order.data.target) > cmpAttack.GetRange(this.order.data.attackType).max)
					{
						let cmpTurAI = Engine.QueryInterface(tur, IID_UnitAI);
						if(cmpTurAI){
							//let cmpTimer = Engine.QueryInterface(SYSTEM_ENTITY, IID_Timer);
							//var data = {"target": this.order.data.target, "min": cmpAttack.GetRange(this.order.data.attackType).max - 1, "max": cmpAttack.GetRange(this.order.data.attackType).max};
							//cmpTimer.SetTimeout(tur, IID_UnitAI, "MoveToTargetRangeExplicitt", 1, data);
							let cmpTargetPosition = Engine.QueryInterface(this.order.data.target, IID_Position);
							let pos = cmpTargetPosition.GetPosition2D();
							cmpTurAI.MoveToPointRange(pos.x, pos.y, cmpAttack.GetRange(this.order.data.attackType).max -1, cmpAttack.GetRange(this.order.data.attackType).max);
						}
						this.FinishOrder();
						return false;
					}
		...
		"GATHER": {
			...
			"GATHERING": {
				"enter": function() {
					...
					this.FaceTowardsTarget(this.order.data.target);
					if (!cmpResourceGatherer.StartGathering(this.order.data.target, IID_UnitAI, this.order.data.full))
					{
						this.ProcessMessage("TargetInvalidated");
						return true;
					}
			...
			"FINDINGNEWTARGET": {
				"enter": function() {
					...
					let redrop = this.order.data.redrop
					let full = this.order.data.full
					...
					let filter = (ent, type, template) => {
						...
						return type.specific == resourceType.specific &&
						    (type.specific != "norm" || resourceTemplate == template);
					};
					...
					if (nearbyResource)
					{
						this.PerformGather(nearbyResource, false, false, false, redrop, full);
						return true;
					}

					// Failing that, try to move there and see if we are more lucky: maybe there are resources in FOW.
...
UnitAI.prototype.AbleToMove = function(cmpUnitMotion)
{
	...
	let Tur = Engine.QueryInterface(tur, IID_UnitAI);
	if (this.isImmobile && tur && Tur.isImmobile)
		return false;
 ...
 */
UnitAI.prototype.FindNearbyResource = function(position, filter, range = 64)
{
	if (!position)
		return undefined;
...
UnitAI.prototype.SetDefaultAnimationVariant = function()
{
	...
	if (cmpResourceGatherer)
	{
		...
		if (type)
		{
			...
			if (type.specific == "norm")
				typename = "carry_meat";// + type.specific;
...
UnitAI.prototype.MoveToTargetRangeExplicitt = function(data, lateness)
{
	this.MoveToTargetRangeExplicit(data.target, data.min, data.max);
}
...
UnitAI.prototype.Walk = function(x, z, queued, pushFront)
{
	...
	if (cmpTurretable)
	{
		...
		if (tur)
		{
			...
			if (cmpTurAI)
			{
				cmpTurAI.Walk(x, z, queued, pushFront);
			}
		}
		else
			mov = 1;
...
UnitAI.prototype.WalkToPointRange = function(x, z, min, max, queued, pushFront)
{
	...
	if (cmpTurretable)
	{
		...
		if (tur)
		{
			...
			if (cmpTurAI)
			{
				cmpTurAI.WalkToPointRange(x, z, queued, pushFront);
			}
		}
		else
			mov = 1;
...
UnitAI.prototype.Stop = function(queued, pushFront)
{
	...
	if (cmpTurretable)
	{
		...
		if (tur)
		{
			...
			if (cmpTurAI)
			{
				cmpTurAI.Stop(queued, pushFront);
			}
		}
		else
			mov = 1;
...
UnitAI.prototype.WalkToTarget = function(target, queued, pushFront)
{
	...
	if (cmpTurretable)
	{
		...
		if (tur)
		{
			...
			if (cmpTurAI)
			{
				cmpTurAI.WalkToTarget(target, queued, pushFront);
			}
		}
		else
			mov = 1;
...
UnitAI.prototype.WalkAndFight = function(x, z, targetClasses, allowCapture = this.DEFAULT_CAPTURE, queued = false, pushFront = false)
{
	...
	if (cmpTurretable)
	{
		...
		if (tur)
		{
			...
			if (cmpTurAI)
			{
				cmpTurAI.WalkAndFight(x, z, targetClasses, allowCapture, queued, pushFront);
			}
		}
		else
			mov = 1;
